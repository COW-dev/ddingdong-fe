<!-- ddingdong-fe 팀 스킬 미러 — 원본: .claude/skills/create-test/SKILL.md (Claude Code 스킬). Codex custom prompt로 수동 호출용. -->
...
- 실행: `apps/web` 안에서 `pnpm exec vitest run <경로>` 또는 루트에서 `pnpm --filter @ddingdong/web test`
...
React Query 내부 동작은 mock하지 않는다(`retry: false`만 render 유틸에서 조정됨). API 응답만 모킹한다 — `_api/queries`, `_api/mutations`가 감싸는 `fetcher.get/post/...` 호출 자체를 mock하면 된다 (예: `vi.mock('@/_api/fetcher', ...)`). `_api/queries/{domain}` 모듈 전체를 mock하면 실제 `queryKey`/`queryFn` 연결이 검증되지 않으므로 하지 않는다.
